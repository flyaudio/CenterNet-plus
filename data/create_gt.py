...
def generate_txtytwth(gt_label, w, h, s):
    """
    :param gt_label:[xmin, ymin, xmax, ymax, cls_id]
    :param w:
    :param h:
    :param s: stride
    :return:
    ...
    c_x = (x2 + x1) / 2 * w #center of bbox(metric:pixel)
    c_y = (y2 + y1) / 2 * h
    box_w = (x2 - x1) * w #size of bbox(metric:pixel)
    box_h = (y2 - y1) * h
    ...
    tx = c_x_s - grid_x
    ty = c_y_s - grid_y
    tw = np.log(box_w_s) #todo,why log??
    th = np.log(box_h_s)
    weight = 2.0 - (box_w / w) * (box_h / h) #面积越大,权重越小
...
def gt_creator(img_size, stride, num_classes, label_lists=[]):
    """
    :param label_lists: e.g  [[xmin, ymin, xmax, ymax, cls_id]]
    :return:
    """
    ...
    hs = h // stride
    s = stride
    # gt_tensor = np.zeros([hs, ws, num_classes+4+1+4])#4:tx/ty/w/h;  1:weight;  4:
    gt_tensor = np.zeros([hs, ws, num_classes+4+1])#4:tx/ty/w/h;  1:weight;  4:
